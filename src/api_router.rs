...
pub trait Buildable {
    fn build(registry: &mut GlobalTypeRegistry, pos: Postion) -> RouteComponentType;
}


impl<T: TypescriptType> Buildable for Query<T> {
    fn build(registry: &mut GlobalTypeRegistry, pos: Postion) -> RouteComponentType {
        return RouteComponentType::Query(T::get_definition(registry))
    }
}

impl<T: TypescriptType> Buildable for Json<T> {
    fn build(registry: &mut GlobalTypeRegistry, pos: Postion) -> RouteComponentType {
        return RouteComponentType::Json(pos, T::get_definition(registry))
    }
}

impl<T: TypescriptType + 'static, R: TypescriptType + 'static> Buildable for Result<Json<T>, R> {
    fn build(registry: &mut GlobalTypeRegistry, pos: Postion) -> RouteComponentType {
        return RouteComponentType::Json(pos, Result::<T, R>::get_definition(registry))
    }
}
...
T2: Buildable, 
{
    fn build(registry: &mut GlobalTypeRegistry) -> Vec<RouteComponentType> {
        ...
        let t3 = Res::build(registry, Postion::Result);
